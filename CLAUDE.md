...
- **Duplicating Classify/Execute Mode procedural body back into meta.md, or duplicating `engine_invocations` / convergence / `references_read` normative definitions across meta.md + references/meta-*.md** — Phase 5 split established that meta.md is the dispatcher (Mode Detection + Mode-bound Reference Load + shared SSOT) and `references/meta-classify.md` + `references/meta-execute.md` are the mode-specific procedural bodies. **Forbidden**: (a) re-adding the Classify Mode or Execute Mode body text into meta.md "for convenience" — meta.md must stay below 5,500 tokens (target 4,500) per SLIM-02 D-06; (b) repeating the `engine_invocations` schema sentence, the convergence verdict normative sentence, or the `references_read` attestation schema in any references/* file — these are SSOT-1/2/3 anchored in meta.md §Final JSON Contract + §Convergence Model. Canary #9 Phase 5 (`skills/tas/runtime/tests/simulate_prompt_slim_diff.py`) guards behavioral invariance across the split via deterministic mock JSON diff against committed baselines in `skills/tas-verify/fixtures/canary-9-baseline-{a,b,c}.json`; `/tas-verify` SSOT-1/2/3 lint blocks (`skills/tas-verify/canaries.md` §SSOT Invariants) guard structural single-source. If SSOT lint fails with count > 1, remove the duplicate from the references file and replace with a pointer to meta.md — do NOT resolve by duplicating further or by anchoring SSOT in a references file.
- **Using `tiktoken` / char-count heuristics instead of Anthropic `count_tokens` API in `scripts/measure-prompt-tokens.py`, or adding `anthropic` to `skills/tas/runtime/requirements.txt`** — Phase 5 SLIM-01 made `measure-prompt-tokens.py` a **dev-only** tool that fails loudly when `anthropic` is missing (exit 1 with stderr guidance) or when `ANTHROPIC_API_KEY` is unset. **Forbidden**: (a) falling back to `tiktoken` (`cl100k_base` or any OpenAI tokenizer) or `char // 4` approximation — Anthropic BPE tokenization differs from OpenAI's; hundreds-of-token drift would invalidate the 5,500 threshold judgment (D-04 rejected alternatives); (b) pip-installing `anthropic` into `skills/tas/runtime/requirements.txt` so the script "just works" — end users of `/tas` must NOT need an `ANTHROPIC_API_KEY`. The script lives at `scripts/measure-prompt-tokens.py` (project root, NOT `skills/tas/runtime/scripts/`) as physical enforcement of the dev-only boundary. Runtime token measurement (MetaAgent self-reporting token counts in its final JSON) is REQUIREMENTS Out of Scope — do not add it.
- **Placing the session worktree anywhere other than `${XDG_CACHE_HOME:-${HOME}/.cache}/tas-sessions/{ts}/<project>/`** — Phase 6 D-01/D-02 + ISO-01: session worktree MUST live in XDG-style cache (NOT `${PROJECT_ROOT}/.tas-sessions/`, NOT `/tmp/`, NOT `mktemp`). The named branch `tas/session-{ts}` provides "reviewable PR" identity — `git worktree add --detach` breaks merge/review semantics and is explicitly forbidden (06-CONTEXT.md D-01 Rejected alternatives). Putting the session inside `${PROJECT_ROOT}` breaks ISO-01 isolation (user's `git status --porcelain` must be delta=0 before/after `/tas`). Canary #10 Phase 1 assertions 2 + 4 (`skills/tas/runtime/tests/simulate_session_isolation.py`) guard path discipline + named-branch creation; Canary #11 Phase 1 assertion 2 (`Tas-Session` trailer grep) assumes this path for forensic retrieval of session commits.
- **Cherry-picking chunk commits into `${PROJECT_ROOT}` HEAD instead of `${SESSION_WORKTREE}` HEAD** — Phase 4 chunk sub-loop (meta-execute.md Phase 2d.5 step 7d) MUST use `git -C "${SESSION_WORKTREE}"` as the merge target (Phase 6 D-07 retargeted from Phase 4's original `${PROJECT_ROOT}`). Cherry-picking into the user's main tree breaks Phase 6 session isolation (ISO-01) — the user's HEAD must remain byte-identical before/after `/tas`. The `${PROJECT_ROOT}` is retained as a git-operations source (e.g., `git -C "${PROJECT_ROOT}" worktree add` during bootstrap) but NEVER as a commit/merge target after Phase 6 bootstrap completes. `PRE_MERGE_SHA` rollback must `reset --hard` on `${SESSION_WORKTREE}` as well (not `${PROJECT_ROOT}`). Canary #8 run on the session worktree (re-invoked by Canary #10 Phase 2 Assertion 6) is the regression guard — if Phase 4 cherry-pick regresses to `${PROJECT_ROOT}` HEAD, both Canary #8 and Canary #10 fail.
- **Auto-bypassing pre-commit hooks via `git commit --no-verify` anywhere in tas runtime** — Phase 7 D-08 3-layer guard: tas respects user-defined pre-commit hooks as part of the reviewable-gate identity. On hook failure, the step HALTs with `halt_reason: pre_commit_hook_failed` (new enum, merge/hook orthogonal domain — Phase 3.1 D-TOPO-05 watchdog/hang enum freeze justified exception; Phase 4 `chunk_merge_conflict` precedent). `--no-verify` MUST NOT appear in `meta-execute.md` step 9.6 heredoc, `meta-execute.md` Phase 2d.5 chunk 7b heredoc, `engine-invocation-protocol.md`, `SKILL.md` Recovery Guidance, or any `Bash({command: ...})` invocation anywhere in `skills/`. User escape hatches (`HUSKY=0`, `SKIP=hook1,hook2`, `LEFTHOOK=0`, `chmod -x .git/hooks/pre-commit`) are user-controlled environment variables — tas does NOT wrap them. Canary #11 Phase 2 Assertion 7 (`simulate_step_commits.py`) greps `git commit[^|]*--no-verify` across `skills/tas/`, `skills/tas-verify/`, `skills/tas-explain/`, `skills/tas-workspace/`, `skills/tas-review/` and requires **0 actual-invocation matches** (prohibition references in this bullet + `SKILL.md` Recovery Korean text + `canaries.md` §Canary #11 assertion body are allowed).
