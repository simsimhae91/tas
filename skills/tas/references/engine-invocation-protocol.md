...
- **cherry-pick target** → `${SESSION_WORKTREE}` (NOT `${PROJECT_ROOT}` — Phase 6 ISO-05). Per-chunk merges land in the session worktree's named branch (`tas/session-{ts}`), keeping the user's main branch untouched. The cherry-pick + `git apply --index --binary` fallback + `PRE_MERGE_SHA` rollback + `chunk_merge_conflict` HALT 4 behaviors are **byte-identical** — only the `git -C` variable changes. The `${WORKSPACE}` and therefore `${CHUNK_PATH}` paths naturally nest INSIDE `${SESSION_WORKTREE}` after Phase 6 SKILL.md Phase 0 bootstrap (no separate substitution needed for chunk worktree paths — CLAUDE.md "chunks must live at `$(cd ${WORKSPACE} && pwd)/chunks/chunk-N/`" rule preserved).
